...
class CoaxialRenderer:
    ...
    def render(
        self, pose: np.ndarray, with_depth: bool = False
    ) -> tuple[np.ndarray, np.ndarray | None]:
        """Render at the given camera-to-world pose.

        Returns ``(rgb_uint8, depth_mm | None)``. ``depth_mm`` is ``None``
        unless ``with_depth=True`` — reading the supersampled depth texture
        is the dominant per-frame cost on Windows + moderngl, so it's
        opt-in.
